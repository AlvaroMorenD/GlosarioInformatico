# Glosario Informático

Bienvenido al **Glosario Informático**. Este repositorio contiene una lista de términos y conceptos clave en el ámbito de las redes y la informática, explicados de manera sencilla.

## Índice

- [IP (Protocolo de Internet)](ip.md)
- [Dominio](dominio.md)
- [URL (Localizador Uniforme de Recursos)](url.md)
- [Puerto (21, 22, 80, 443)](puertos.md)
- [Servicio de Directorio](servicioDirectorio.md)
- [Diferencia entre HTTP y HTTPS](httpYhttps.md)
- [Servicio DNS (Sistema de Nombres de Dominio)](DNS.md)
- [Servicio Web (Apache, Nginx)](servicioWeb.md)
- [Protocolo](protocolo.md)
- [RDP (Protocolo de Escritorio Remoto)](RDP.md)
- [SSH (Secure Shell)](SSH.md)
- [Cloud](cloud.md)
- [AWS (Amazon Web Services)](AWS.md)
- [Proxmox](proxmox.md)
- [Google Cloud](GoogleCloud.md)
- [Azure](azure.md)
- [Administrador](administrador.md)
- [DevOps](devops.md)
